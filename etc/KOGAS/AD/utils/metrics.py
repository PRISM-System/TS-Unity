...
이 모듈은 이상 탐지 모델의 성능 평가를 위한 다양한 메트릭 함수들을 제공합니다.
"""
...
def RSE(pred: np.ndarray, true: np.ndarray) -> float:
    """
    Relative Squared Error (상대 제곱 오차)를 계산합니다.
...
def MAE(pred: np.ndarray, true: np.ndarray) -> float:
    """
    Mean Absolute Error (평균 절대 오차)를 계산합니다.
...
def MSE(pred: np.ndarray, true: np.ndarray) -> float:
    """
    Mean Squared Error (평균 제곱 오차)를 계산합니다.
...
def RMSE(pred: np.ndarray, true: np.ndarray) -> float:
    """
    Root Mean Squared Error (평균 제곱근 오차)를 계산합니다.
...
def MAPE(pred: np.ndarray, true: np.ndarray) -> float:
    """
    Mean Absolute Percentage Error (평균 절대 백분율 오차)를 계산합니다.
...
def MSPE(pred: np.ndarray, true: np.ndarray) -> float:
    """
    Mean Squared Percentage Error (평균 제곱 백분율 오차)를 계산합니다.
...
def calc_point2point(predict: np.ndarray, actual: np.ndarray) -> Tuple[float, float, float, int, int, int, int]:
    """
    예측값과 실제값을 기반으로 F1 점수를 계산합니다.
    ...
    Returns:
        (F1, precision, recall, TP, TN, FP, FN) 튜플
    """
...
def adjust_predicts(score: np.ndarray, label: np.ndarray,
                   threshold: Optional[float] = None,
                   pred: Optional[np.ndarray] = None,
                   calc_latency: bool = False) -> np.ndarray:
    """
    주어진 점수, 임계값(또는 예측값) 및 라벨을 사용하여 조정된 예측 라벨을 계산합니다.
    
    Args:
        score: 이상 점수
        label: 실제 라벨
        threshold: 이상 점수 임계값 (점수가 임계값보다 낮으면 "이상"으로 라벨링)
        pred: 예측 라벨 (None이 아닌 경우 score와 threshold를 무시하고 pred를 조정)
        calc_latency: 지연 시간 계산 여부
        
    Returns:
        예측 라벨 배열
        
    Raises:
        ValueError: score와 label의 길이가 다른 경우
    """
    if len(score) != len(label):
        raise ValueError("score와 label은 같은 길이여야 합니다")
